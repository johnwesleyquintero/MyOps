      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredReflections.map((reflection) => (
            <div
              key={reflection.id}
              className="group relative bg-notion-light-sidebar dark:bg-notion-dark-sidebar p-6 rounded-2xl border border-notion-light-border dark:border-notion-dark-border hover:border-blue-500/30 transition-all hover:shadow-xl hover:shadow-blue-500/5 flex flex-col gap-4 cursor-pointer"
              ...
              {(reflection.learnings?.length > 0 ||
                ...
                <div className="pt-4 border-t border-notion-light-border dark:border-notion-dark-border mt-auto flex flex-col gap-3">
                  {reflection.learnings?.length > 0 && (
                    <div className="flex flex-col gap-1.5">
                      <span className="text-[10px] font-semibold text-notion-light-muted dark:text-notion-dark-muted flex items-center gap-1.5">
                        <Icon.Active size={10} className="text-emerald-500" />
                        LEARNINGS
                      </span>
                      <div className="flex flex-wrap gap-1.5">
                        {reflection.learnings.slice(0, 2).map((l, i) => (
                          <span
                            key={i}
                            className="px-2 py-0.5 bg-emerald-500/5 text-emerald-600 dark:text-emerald-400 rounded-full text-[9px] font-medium border border-emerald-500/10 max-w-full truncate"
                          >
                            {l}
                          </span>
                        ))}
                  ...
                  {reflection.actionItems?.length > 0 && (
                    <div className="flex flex-col gap-1.5">
                      <span className="text-[10px] font-semibold text-notion-light-muted dark:text-notion-dark-muted flex items-center gap-1.5">
                        <Icon.Bot size={10} className="text-violet-500" />
                        ACTION ITEMS
                      </span>
                      <div className="flex flex-wrap gap-1.5">
                        {reflection.actionItems.slice(0, 2).map((a, i) => (
                          <span
                            key={i}
                            className="px-2 py-0.5 bg-violet-500/5 text-violet-600 dark:text-violet-400 rounded-full text-[9px] font-medium border border-violet-500/10 max-w-full truncate"
                          >
                            {a}
                          </span>
                        ))}
