...
export const generateSVGPoints = (chartData: { date: string, val: number }[], width: number, height: number, padding: number): string => {
...
export const calculateDailyTrend = (entries: TaskEntry[]): { date: string; val: number }[] => {
...
export const calculateTopExpenses = (entries: TaskEntry[]): { cat: string; val: number; pct: number }[] => {
