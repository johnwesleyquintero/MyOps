...
import { MentalStateEntry } from "@/types";
...
interface AwarenessViewProps {
  ...
  isLoading: boolean;
  onSave: (entry: MentalStateEntry) => Promise<MentalStateEntry>;
}

export const AwarenessView: React.FC<AwarenessViewProps> = React.memo(
  ({ history, isLoading, onSave }) => {
    ...
    // Sync with existing entry for today if it exists
    useEffect(() => {
      const today = new Date().toISOString().split("T")[0];
      const existing = history.find((e) => e.date === today);
      if (existing && existing.id !== todayEntry.id) {
        const timeoutId = setTimeout(() => {
          setTodayEntry(existing);
        }, 0);
        return () => clearTimeout(timeoutId);
      }
    }, [history, todayEntry.id]);
