    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      ...
      >
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wide">Keyboard Shortcuts</h3>
        ...
        <div className="p-6 grid gap-6">
          {groups.map((group) => (
            <div key={group.title}>
              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">{group.title}</h4>
                  ...
                  <div key={idx} className="flex justify-between items-center text-sm">
                    ...
                    <div className="flex gap-1">
                      {item.keys.map((k, kIdx) => (
                        <kbd key={kIdx} className="bg-slate-100 border border-slate-300 rounded px-2 py-0.5 text-xs font-mono font-bold text-slate-600 min-w-[24px] text-center shadow-sm">
                          {k}
                        </kbd>
                      ))}
                    </div>
